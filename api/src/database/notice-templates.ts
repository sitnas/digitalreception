...
function body(cc: string, lang: Lang): string {
  ...
  if (lang === 'it') return [
    ...
    `Dati trattati: nome, cognome, azienda di provenienza, email (facoltativa), persona di riferimento, motivo della visita, orari di ingresso e uscita, firma. Ove previsto dalla sede: tipo e numero del documento di identità, foto del documento, foto del numero di serie del computer portatile.`,
    `Finalità: controllo degli accessi e sicurezza di persone e beni, gestione delle emergenze e dell'evacuazione (elenco dei presenti), invio della presente informativa.`,
  ...
  if (lang === 'es') return [
    ...
    `Datos tratados: nombre, apellidos, empresa, correo electrónico (opcional), persona de contacto, motivo de la visita, horas de entrada y salida y firma. Cuando la sede lo requiera: tipo y número de documento de identidad, foto del documento y foto del número de serie del portátil.`,
    `Finalidad: control de accesos, seguridad de personas y bienes, gestión de emergencias y evacuación (listado de presentes) y envío de esta información.`,
    `Base jurídica: ${basis}`,
  ...
  return [
    ...
    `Data processed: first and last name, company, email (optional), host, purpose of visit, entry and exit times, signature. Where required by the site: identity document type and number, document photo, photo of the laptop serial number.`,
    `Purposes: access control and security of people and property, emergency and evacuation management (list of people on site), delivery of this notice.`,
    `Legal basis: ${basis}`,
