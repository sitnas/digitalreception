...
function body(cc: string, lang: Lang): string {
  ...
  if (lang === 'it') return [
    ...
    `Finalità: controllo degli accessi e sicurezza di persone e beni, gestione delle emergenze e dell'evacuazione (elenco dei presenti), statistiche aggregate sulla provenienza dei visitatori, avviso di arrivo alla persona di riferimento, invio via email del badge di uscita e, su richiesta, della presente informativa.`,
  ...
  if (lang === 'es') return [
    ...
    `Finalidad: control de accesos, seguridad de personas y bienes, gestión de emergencias y evacuación (listado de presentes), estadísticas agregadas sobre la procedencia de los visitantes, aviso de llegada a la persona de contacto, envío por correo de la credencial de salida y, si se solicita, de esta información.`,
    `Base jurídica: ${basis}`,
  ...
  return [
    ...
    `Purposes: access control and security of people and property, emergency and evacuation management (list of people on site), aggregate statistics on where visitors come from, notifying the host of your arrival, emailing the exit badge and, on request, this notice.`,
    `Legal basis: ${basis}`,
